...
def test_extract_json_with_messages():
    json_msg = json.dumps({"response": {"type": "ok", "messages": [{"from": "user", "entry": "Hi", "timestamp": "2023-10-01T12:00:00Z"}]}})
    result = extract_json(json_msg)
    assert result.type == "ok" and result.messages == [{"from": "user", "entry": "Hi", "timestamp": "2023-10-01T12:00:00Z"}]
...
def test_connect_success(mock_socket):
    mock_socket.return_value.makefile.return_value.readline.return_value = json.dumps({"response": {"type": "ok", "token": "test_token"}})
...
def test_connect_failure(mock_socket):
    mock_socket.return_value.makefile.return_value.readline.return_value = json.dumps({"response": {"type": "error"}})
...
def test_send_message_success(mock_socket):
    mock_socket.return_value.makefile.return_value.readline.side_effect = [
        ...
        json.dumps({"response": {"type": "ok", "message": "Direct message sent"}})
    ]
    dm = DirectMessenger("test_server", "test_user", "test_password")
...
def test_retrieve_new(mock_socket):
    mock_socket.return_value.makefile.return_value.readline.side_effect = [
        json.dumps({"response": {"type": "ok", "token": "test_token"}}),
        json.dumps({"response": {"type": "ok", "messages": [{"from": "user", "entry": "Hi", "timestamp": "2023-10-01T12:00:00Z"}]}})
    ]
    dm = DirectMessenger("test_server", "test_user", "test_password")
...
def test_close_connection(mock_socket):
    ...
    mock_socket_instance.makefile.return_value.readline.return_value = json.dumps({"response": {"type": "ok", "token": "test_token"}})
    dm = DirectMessenger("test_server", "test_user", "test_password")
    dm.close_connection()
